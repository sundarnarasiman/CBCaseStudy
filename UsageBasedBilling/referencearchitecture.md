...
## 10. Implementation Details: API Gateway

### 10.1 Overview
The **API Gateway** is the front-facing entry point, implemented in **FastAPI**. It performs strict schema validation of incoming payload models, generates idempotency keys when missing, and publishes usage events to the `usage-events` Kafka topic.
...
sequenceDiagram
    ...
    API->>API: Generate idempotencyKey
    API->>Kafka: produce(topic, message)
    Kafka-->>API: ACK
...
classDiagram
    ...
    APIGateway --> UsageEvent : validates
    APIGateway --> KafkaProducer : pushes to
```
...
erDiagram
    API_USAGE_EVENT {
        ...
        string event
        string timestamp
        string idempotencyKey "Autogenerated if empty"
    }
```

## 11. Implementation Details: Notification System

### 11.1 Overview
The **Notification System** is an independent consumer that listens to the `notifications` Kafka topic. Whenever significant events occur (e.g., threshold alerts, invoice generation), the system parses the message and dispatches an email using SMTP.

### 11.2 Sequence Diagram
```mermaid
sequenceDiagram
    participant Kafka as Notification Stream
    participant Notifier as NotificationProcessor
    participant SMTP as SMTP Server

    Kafka->>Notifier: Poll notification event
    Notifier->>Notifier: Extract email, subject, body
    Notifier->>SMTP: send_message(msg)
    SMTP-->>Notifier: OK
```
...
classDiagram
    ...
    NotificationProcessor --> SMTP : sends mail via
```
...
flowchart TD
    ...
    N1 --> SMTP[SMTP Gateway / SendGrid]
```

### 11.5 Data Model
```mermaid
erDiagram
    KAFKA_NOTIFICATION_PAYLOAD {
        string email
        string subject
        string body
    }
...
## 12. Implementation Details: Idempotency Layer

### 12.1 Overview
The **Idempotency Layer** serves as a guardrail against duplicate event submissions (e.g., from network retries). It sits between the raw `ingress-events` topic and the finalized `usage-events` topic. It uses Redis `SETNX` to determine if an event's `idempotencyKey` has been seen recently, forwarding it only if it is unique.

### 12.2 Sequence Diagram
```mermaid
sequenceDiagram
    participant KafkaIn as Kafka (ingress-events)
    participant Idem as IdempotencyProcessor
    participant Redis as Redis Cache
    participant KafkaOut as Kafka (usage-events)

    KafkaIn->>Idem: Poll event
    Idem->>Redis: SETNX idempotent:{key} "1"
    alt Duplicate
        Redis-->>Idem: 0
        Idem->>Idem: Drop event
    else New
        Redis-->>Idem: 1
        Idem->>Redis: EXPIRE idempotent:{key} 604800
        Idem->>KafkaOut: Forward event
    end
```
...
classDiagram
    class IdempotencyProcessor {
        ...
        -Producer producer
        -Redis redisClient
        +process_event(event_data) bool
        +forward_event(event_data) bool
        +run()
    }
    IdempotencyProcessor --> Redis : uses
```
...
flowchart TD
    subgraph K8s [Kubernetes]
        I1[Idempotency Worker 1]
    end
    Kafka1[(Topic: ingress-events)] --> I1
    I1 --> Redis[(Redis Cluster)]
    I1 --> Kafka2[(Topic: usage-events)]
```
...
erDiagram
    REDIS_IDEMPOTENCY_KEY {
        string key PK "idempotent:{idempotencyKey}"
        string value "1"
        int ttl "7 days"
    }
```

## 13. Implementation Details: Event Sources

### 13.1 Overview
The **Event Sources** component acts as a mock client representing external systems (telemetry, IoT, etc.) that inject heavy traffic directly into the data pipeline (via Kafka) for load testing and functional validation.

### 13.2 Sequence Diagram
```mermaid
sequenceDiagram
    participant Generator
    participant Kafka as Topic (ingress-events)

    loop Every second
        Generator->>Generator: create random payload
        Generator->>Kafka: Produce event
        Kafka-->>Generator: ACK
    end
```
...
classDiagram
    ...
    EventGenerator --> KafkaProducer : utilizes
```
...
## 14. Implementation Details: External Integrations

### 14.1 Overview
The **External Integrations** module acts as a synchronization bridge between the Usage-Based Billing platform and third-party CRMs or ERPs (e.g., Salesforce, NetSuite). It is implemented as a FastAPI background worker listening to finalized events.

### 14.2 Sequence Diagram
```mermaid
sequenceDiagram
    participant Kafka as Topic (invoices-generated)
    participant Ext as ExternalIntegrationWorker
    participant CRM as Third-Party CRM

    Kafka->>Ext: Poll finalized invoice
    Ext->>CRM: API Call (Sync Invoice Data)
    CRM-->>Ext: 200 OK
```
